...
class SentimentClassifier:
	...
	def classify_percentages(self, tweet):
		...
		#total_count = 0
		#for feat in features:
		#NaiveLabels = [self._classifier.classify(features)]
		NaiveLabels = [self._classifier.classify(self.extract_features(tweet[0])) for tweet in tokens]
		#print(NaiveLabels)	
		#if probability.max() == 'positive':
		pos_count = NaiveLabels.count('positive')
				
		#else:
		neg_count = NaiveLabels.count('negative')

		#total_count = pos_count + neg_count	
		pos_percent = round((100 * pos_count / len(NaiveLabels)), 1)
		neg_percent = round((100 * neg_count / len(NaiveLabels)), 1)
